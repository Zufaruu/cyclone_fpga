...
catch {set coef_files [glob -nocomplain bandpass_coef*.txt]} error_msg
puts "$error_msg"
  
# The top-level in HDL type "VERILOG"
set ipfs_ext vo
if {[file exists bandpass.vo]} {
	set hdl_ext v
} else {
	puts "Warning: Could not find bandpass.$ipfs_ext!"
}
...
set_global_assignment -name EDA_TEST_BENCH_MODULE_NAME work.tb_bandpass -section_id tb
set_global_assignment -name EDA_TEST_BENCH_GATE_LEVEL_NETLIST_LIBRARY work -section_id tb

# IPFS file
set_global_assignment -name EDA_IPFS_FILE bandpass.$ipfs_ext -section_id eda_simulation -library work
...
set_global_assignment -name EDA_TEST_BENCH_FILE bandpass_input.txt -section_id tb -library work

set_global_assignment -name EDA_TEST_BENCH_FILE tb_bandpass.vhd -section_id tb -library work
